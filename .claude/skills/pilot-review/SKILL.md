...
1. Resolve the PR(s) per `docs/pilot-process.md` §6 step 0: the given PR/issue number, or
   ...
   (`docs/pilot-process.md` §6): PM — product fit against the story's acceptance criteria
   (`type:feature`) or whether the test actually validates the story's end-to-end flow
   (`type:e2e`); architect — conformance to the security/architecture decisions recorded
   at scope time; tech lead — both conformance to its own spec *and* general code
   quality/maintainability. If a future edit changes what any one agent's own file says it
   checks, re-verify this list still adds up to full coverage.
...
3. Call the `Agent` tool once per reviewer, **in parallel** (independent calls in the
   ...
   PM, the linked story's acceptance criteria (and, for a `type:e2e` task, its own spec
   too); for the architect, its own recorded
