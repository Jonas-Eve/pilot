...
1. Determine the input:
   ...
   - Any other issue number: must be `status:qa` (else — `status:split`, `status:done`, a
     type that never reaches `status:qa`, `docs/pilot-process.md` §7 "When it fires") —
     report and stop rather than claiming it.
   - No argument → per `docs/pilot-process.md` §4 "Picking the next ticket...": merged
     ...
     excluding `on-hold`, highest `priority:` then oldest first. No `--auto`, never
     Routine-driven (`docs/pilot-process.md` §4 "Interaction modes", "Scheduled sweeps").
...
6. Apply the agent's verdict:
   ...
   - **One or more real-bug failures**: nothing further to set — the agent already
     ...
     (never through phase 2, `docs/pilot-process.md` §2 "Three levels"), and
     self-applied `on-hold` with its own comment (`status:in-qa` stays), the same pattern
     ...
     6)"). Takes priority over a same-pass "not actually a bug" failure — story stays
     `in-qa` regardless. Becomes a candidate again only via explicit `--resume`, once a
     human removes `on-hold` after the new bug ticket reaches `status:done`.
