---
name: pilot-qa
description: "Phase 6 of PILOT (see docs/pilot-process.md): human QA for a type:feature story once every sub-ticket — including its mandatory e2e one — has merged (status:qa). Builds a manual test plan from the story's acceptance criteria and its merged sub-tickets, walks a human through testing it live, and reports the verdict. Pair-only, no --auto — there's no unattended version of a human testing something, so this is never Routine-driven. On a full pass, sets status:done and closes the issue. On a failure, adds needs-human with exactly what failed (the bug-handling flow for this case is still being designed — a human decides what happens next by hand for now). Also resumes a ticket left mid-pair session with --resume <issue number>, and runs bare with no argument to pick up the next status:qa (fresh) or status:in-qa-with-needs-human-cleared (resumable) ticket, skipping anything still on-hold. Use once a type:feature story's sub-tickets have all merged and it's ready for a human to confirm the shipped behavior."
...
3. Gather context: the story's own body (acceptance criteria) and, for each of its dev and
   e2e sub-tickets, the spec (phase 3) and the merged PR (`mcp__github__issue_read`,
...
6. Apply the agent's verdict:
   - **Approved** (every case confirmed): set `status:done` and close the issue
     ...
     `type:feature` story is never itself a sub-ticket of another `status:split` parent.
...
Do not run this against a `type:tech`/`type:bug` ticket, or against a sub-ticket itself —
