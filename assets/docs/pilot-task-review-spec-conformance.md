...
the same way (`docs/pilot-link-review-consensus.md`).
...
- **Code quality/maintainability** — readability/naming, whether tests actually exercise
  ...
  implementation commit(s) that fix it (`docs/pilot-task-implement.md` step 3). If the history
  ...
  (`docs/pilot-task-implement-e2e.md`) — judge them instead on whether the test exercises the
  real, already-merged integration points it claims to, without mocking them away.
  There's no separate reviewer here: `pilot-dev` already self-reviewed the diff before
  opening the PR (`docs/pilot-process.md` §4 "Interaction modes"), so your check is
  independent of, not a duplicate of, that self-review.
