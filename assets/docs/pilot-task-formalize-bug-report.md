...
discovery (`.pilot/pilot-link-bug-tickets.md`) rather than through `/pilot-story` directly.
...
`status:spec-ready` (`.pilot/pilot-process.md` §2 "Three levels") — no priority or splitting
decision to defer.
...
1. Confirm it's actually a defect against already-agreed behavior (regression, broken
   ...
   outcome `/pilot-story` already handles for any raw need (`.pilot/pilot-process.md` §4
   "Interaction modes").
...
4. Label it `type:bug`, `level:task`, `status:spec-ready`, unassigned, with `priority:`
   set yourself (`.pilot/pilot-process.md` §3 — every ticket gets its initial priority at
