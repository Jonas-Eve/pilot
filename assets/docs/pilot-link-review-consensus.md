...
against the PR's branch before voting (`docs/pilot-task-review-spec-conformance.md`) — the
...
- At least one blocking point tagged `change`, **none** tagged `decision` → a request for
  ...
  `docs/pilot-process.md` §4 "Reclaiming a `status:changes-requested` ticket" — that
