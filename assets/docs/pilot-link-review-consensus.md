...
which carry only identity now. See `.pilot/pilot-process.md` §2/§3 for the generic
labels/states and `.pilot/pilot-link-claim-protocol.md` for the claim protocol this builds
on.
...
A validation/CI mismatch always counts as `change` — the tech lead is the one who checks
it (`.pilot/pilot-task-review-spec-conformance.md`), so the other reviewers never duplicate
that check themselves — and once this project has a CI workflow covering the affected
area, a red or pending check on the PR's head commit is the same kind of automatic block;
until then, the tech lead's own check is PILOT's only safety net. Once
